...
def plot_time_trend(df, title, x_col="date", y_col="count"):
    """
    ...
    px.line(year_month, x=x_col, y=y_col, title=title)
...
def plot_scatters(group, group_names, title, filter=False, filter_list=None, file_path=None):
    """
    ...
    assert isinstance(filter_list, list) or filter is None
    assert isinstance(file_path, str) or file_path is None
    assert isinstance(title, str)
    fig = go.Figure()
    for i in range(len(group_names)):
        if filter and group_names[i] in filter_list:
            continue
    ...
    fig.update_layout(legend=dict(font=dict(family="Times", size=25, color="black")))
    fig.update_layout(
        xaxis_title="Date",
        yaxis_title="Count",
        font=dict(
